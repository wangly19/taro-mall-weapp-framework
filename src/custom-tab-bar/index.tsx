...
const CustomTabbar = () => {
  const selectedPath: string = useSelector(
    (state: RootState) => state.common.selectTabBarPath,
    shallowEqual
  );
        ...
        return (
          ...
          >
            {item.iconPath && item.selectedIconPath ? (
              <CoverImage
                className="tab-bar__item__icon"
                src={isSelected ? item.selectedIconPath : item.iconPath}
              />
            ) : null}
            <CoverView
              className="tab-bar__item__text"
              onClick={switchTab(item.pagePath)}
            >
              {item.text}
            </CoverView>
